...
export default async function MembersPage() {
  ...
  const memberRows = (members ?? []).map((member) => ({
    ...member,
    patrol_name: (member.patrols as { name?: string } | null)?.name ?? '-',
  }))
  return <div><PageHeading eyebrow="Peserta Didik" title="Manajemen Anggota" description="Tambah, cari, dan perbarui profil anggota. Data ini menjadi dasar pembagian regu, absensi, dan penilaian SKU." /><CrudManager entity="member" activeGudepId={context.activeGudep.id} items={memberRows as Array<Record<string, unknown>>} createLabel="Tambah Anggota" emptyTitle="Belum ada anggota" emptyDescription="Masukkan anggota pertama atau gunakan form secara bertahap." fields={[
    ...
    { key: 'name', label: 'Nama' }, { key: 'class_name', label: 'Kelas' }, { key: 'gender', label: 'L/P' }, { key: 'scout_level', label: 'Tingkat' }, { key: 'patrol_name', label: 'Regu' }, { key: 'status', label: 'Status' },
  ]} /></div>
}
