...
export default async function ReportsPage() {
  ...
  const [members, sessions, attendance, sku, skk] = await Promise.all([
    ...
    supabase.from('member_skk_progress').select('level,status,note,validated_at,members!inner(name,class_name,gudep_id),skk_items(field_number,item_number,name,field_name)').eq('members.gudep_id', gudepId),
  ])
  const datasets = [
    ...
    { name: 'skk', label: 'Progress SKK', rows: (skk.data ?? []).map((row) => { const member = firstRelated(row.members); const item = firstRelated(row.skk_items); return { Nama: member?.name ?? '', Kelas: member?.class_name ?? '', Bidang: item?.field_number ?? '', 'Nama Bidang': item?.field_name ?? '', Nomor: item?.item_number ?? '', SKK: item?.name ?? '', Tingkat: row.level, Status: row.status, Catatan: row.note, 'Tanggal Validasi': row.validated_at } }) },
  ]
  return <div><PageHeading eyebrow="Administrasi" title="Pusat Laporan" description="Unduh data anggota, latihan, absensi, SKU, dan SKK sebagai CSV yang dapat langsung dibuka di Microsoft Excel atau Google Sheets." /><ReportCenter datasets={datasets} /></div>
}
