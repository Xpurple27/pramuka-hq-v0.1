...
type Props = { entity: Entity; items: Array<Record<string, unknown>>; fields: Field[]; columns: Array<{ key: string; label: string }>; activeGudepId?: string; createLabel: string; emptyTitle: string; emptyDescription: string }
...
export default function CrudManager({ entity, items, fields, columns, activeGudepId, createLabel, emptyTitle, emptyDescription }: Props) {
  ...
  return <div className="space-y-4">
    ...
    {filtered.length === 0 ? <div className="card p-10 text-center"><h3 className="font-bold text-white">{emptyTitle}</h3><p className="mt-2 text-sm text-slate-500">{emptyDescription}</p><button onClick={openCreate} className="btn-primary mt-5"><Plus className="h-4 w-4" />{createLabel}</button></div> : <div className="card overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="border-b border-white/8 bg-white/[0.02] text-xs uppercase tracking-wider text-slate-500"><tr>{columns.map((column) => <th key={column.key} className="px-4 py-3 font-bold">{column.label}</th>)}<th className="px-4 py-3 text-right">Aksi</th></tr></thead><tbody className="divide-y divide-white/5">{filtered.map((item) => <tr key={String(item.id)} className="text-slate-300 hover:bg-white/[0.02]">{columns.map((column) => <td key={column.key} className="px-4 py-3.5">{String(item[column.key] ?? '-')}</td>)}<td className="px-4 py-3.5"><div className="flex justify-end gap-2"><button aria-label="Edit" onClick={() => openEdit(item)} className="icon-button"><Edit3 className="h-4 w-4" /></button><button aria-label="Hapus" onClick={() => remove(item)} disabled={isPending} className="icon-button text-red-300"><Trash2 className="h-4 w-4" /></button></div></td></tr>)}</tbody></table></div></div>}
